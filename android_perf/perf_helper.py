...
class AppPerfBaseHelper(metaclass=abc.ABCMeta):
    ...
    def get_memory_usage(self, pid=None, pid_list=None):
        if pid:
            return self.adb.get_process_memory(pid)
        return self.adb.get_processes_memory(pid_list or self.adb.find_process_ids(self.app.pkg))
    ...
    def _async_on_test_cpu_memory(self, current_second: int, max_listen_seconds: int, min_wait_seconds: int,
        ...
        if len(raw_data) < min_wait_seconds:
            # 取数少于n秒的不进行后续操作
            return
        self._compute_cpu_memory(raw_data, data)
        self.on_test_cpu_memory(current_second, max_listen_seconds, data)
    ...
    def start_test_cpu_memory(self, min_wait_seconds: int = 10, max_listen_seconds: int = 60) -> dict:
        ...
        self.before_test()
        tmp_data = {
            int(time.time() * 1000): {'cpu_g': self.adb.get_cpu_global(), 'cpu_a': AppCPU(0, 0), 'memory': 0}
        ...
        for i in range(max_listen_seconds):
            ...
            # 1秒读一次数据
            time.sleep(1)
            self._th_pool.putRequest(WorkRequest(self._async_run_get_cpu_memory, args=(main_pid, tmp_data)))
